...
import pandas as pd
import math
from Mission_reliability_dashboard.taskRelcode import TaskRelCode
from Mission_reliability_dashboard import queries as tq
...
class TaskReliability:
    ...
    def lmu_rel(self, mission_name, system, platform, total_dur, c_age=0):
        ...
        cursor.execute(tq.GET_ALPHA_BETA_JOIN, (system, platform))
        alpha_beta_data = cursor.fetchall()

        eta_beta_data = []
        if len(alpha_beta_data) == 0:
            cursor.execute(tq.GET_ETA_BETA_JOIN, (system, platform))
            eta_beta_data = cursor.fetchall()

        cursor.execute(tq.GET_SYSTEM_CONFIG, (platform, system))
        ...
        if len(alpha_beta_data) > 0:
            ...
            for lmu in eta_beta_data:
                cursor.execute(tq.GET_LAST_MAINT_DATE, (lmu[5],))
                first_data = cursor.fetchone()
                cursor.execute(tq.GET_PARENT_SHIP_ID, (platform, system))
                ship_id = cursor.fetchone()[0]

                if first_data is None:
                    cursor.execute(tq.GET_AVG_RUNNING, (ship_id,))
                    c_age = cursor.fetchone()[0] or 0
                else:
                    cursor.execute(
                        tq.GET_AVG_RUNNING_AFTER_DATE,
    ...
    def get_curr_age(self, component_id):
        ...
        cursor.execute(tq.GET_LATEST_RUNNING_AGE, (component_id,))
        row = cursor.fetchone()
        if row is None:
            return None
    ...
    def estimate_alpha_beta(self, component_id, component_name):
        instance = AlphaBeta()
        instance_2= OverhaulsAlgos()
        cursor.execute(tq.GET_OVERHAUL_INFO, (component_id,))
        ...
        instance_2.insert_overhauls_data(
            equipment_id=component_id,
            run_age_component=float(run_age_value),
        )

        cursor.execute(
            tq.GET_OVERHAUL_MAINT_DATA_ORDERED,
            (component_id,)
        )
        data = cursor.fetchall()
    ...
    def fetch_alpha_beta(self, component_id):
        ...
        cursor.execute(tq.GET_NOMENCLATURE_BY_ID, (component_id,))
        ...
        cursor.execute(tq.GET_ALPHA_BETA, (component_id,))
        result = cursor.fetchone()
