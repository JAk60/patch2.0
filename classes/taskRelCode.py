...
class TaskRelCode:
    ...
    def top_equipment_in_group(self, group_equip, group_alpha, group_bta, group_t, D, k, n):
        ...
        group_FR = []
        for i in range(n):
            equip_rel, FR = self.equipment_reliability(
                group_alpha[i], group_bta[i], group_t[i], D)
        ...
        for j in range(k):
            ...
            max_rel_equip_index.append(
                group_equi_rel.index(max(group_equi_rel_copy)))
            group_equi_rel_copy.remove(max(group_equi_rel_copy))
        for i in range(n):
            print(
                f"[old_unit] {group_equip[i]}: age={group_t[i]:.1f}, rel={group_equi_rel[i]:.4f}, FR={group_FR[i]:.6f}")
    ...
    def group_rel(self, group_equip, group_alpha, group_bta, group_t, D, k, n):
        max_rel_equip, max_rel_equip_index, group_equi_rel, FR = self.top_equipment_in_group(
            group_equip, group_alpha, group_bta, group_t, D, k, n)
        if (k < n):
            FR_sum = 0
            not_max_equip_index = []
            for j in range(n):
            ...
            Rel = (math.e ** -(k * lamda_max * D)) * sum((((k * lamda_max * D) **
                                                           i) / math.factorial(i)) for i in range(len(not_max_equip_index)))
            print(
                f"[old_kofn] FR_sum={FR_sum}, lamda_max={lamda_max}, kLD={k*lamda_max*D}")
        else:
            Rel = 1
